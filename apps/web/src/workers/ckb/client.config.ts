...
const createConfig = (network: string, bootnodes: string[]) => {
  const config = {
    chain: network,
    store: {
      path: `data/${network}/store`,
    },
    network: {
      path: `data/${network}/network`,
...
export const getLightNodeConfig = (network: "mainnet" | "testnet") => {
  if (network === "mainnet") {
    return createConfig("mainnet", [
