...
import { cccTxToSkeleton, normalize } from "./utils"

const client = new ClientPublicTestnet()
const rpc = new CKBRPC("https://testnet.ckb.dev")
...
describe("transfer", () => {
  it("lumos should match ccc transfer object", async () => {
    ...
    const convertedSkeleton = normalize(cccTxToSkeleton(cccTx))
    const expectedSkeleton = normalize(transactionSkeletonToObject(txSkeleton))

    expect(convertedSkeleton).toEqual(expectedSkeleton)
  })
  ...
  it("tx objects are equal", async () => {
    ...
    } = await prepareResult(client, cccTx, lockToPath)

    expect(normalize(resultTx)).toEqual(normalize(lumosRawTx))
    expect(normalize(resultContexts)).toEqual(normalize(lumosContexts))
    expect(normalize(resultWitnesses)).toEqual(normalize(lumosWitnesses))
  })
})
