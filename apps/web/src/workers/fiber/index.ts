...
let currentFiberKeyPairHex: string | null = null
...
async function getFiberClient(): Promise<Fiber> {
  if (!currentNetwork || !currentFiberKeyPairHex) {
    throw new Error("Worker not initialized. Call UPDATE_CONFIG first.")
  }

  const { startFiberClient } = await import("./client")
  return startFiberClient(currentNetwork, currentFiberKeyPairHex, currentClientMode)
}
...
async function handleMessage(request: WorkerRequest): Promise<WorkerTypeMap[WorkerMethod]["result"]> {
  ...
  if (method === "UPDATE_CONFIG") {
    ...
    currentFiberKeyPairHex = payload.fiberKeyPairHex
  ...
  switch (method) {
    case "CONNECT_PEER":
      return withTimeout(fiber.connectPeer(payload), 20_000, "CONNECT_PEER")

    case "LIST_PEERS":
      return withTimeout(fiber.listPeers(), 10_000, "LIST_PEERS")

    case "OPEN_CHANNEL_WITH_EXTERNAL_FUNDING":
      return withTimeout(fiber.openChannelWithExternalFunding(payload), 30_000, "OPEN_CHANNEL_WITH_EXTERNAL_FUNDING")

    case "SUBMIT_SIGNED_FUNDING_TX":
      return withTimeout(fiber.submitSignedFundingTx(payload), 30_000, "SUBMIT_SIGNED_FUNDING_TX")

    case "CLOSE_CHANNEL":
      return withTimeout(fiber.shutdownChannel(payload), 20_000, "CLOSE_CHANNEL")

    case "ABANDON_CHANNEL":
      return withTimeout(fiber.abandonChannel(payload), 20_000, "ABANDON_CHANNEL")

    case "CREATE_INVOICE":
      return withTimeout(fiber.newInvoice(payload), 10_000, "CREATE_INVOICE")

    case "LIST_CHANNELS":
      return withTimeout(fiber.listChannels(payload), 10_000, "LIST_CHANNELS")
